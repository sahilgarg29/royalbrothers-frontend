import "./citySearch.css"
import { FaSearch } from 'react-icons/fa';
import React from "react"

function CitySearch(){
    const [cityData, setCityData] = React.useState([])
    React.useEffect(()=>{
        fetch("https://royalbrothers-backend.herokuapp.com/api/cities")
        .then((res) => res.json())
        .then((res) => {
          setCityData(res.cities);
        })
    },[])

     function cityNameFun(e){
         console.log(e)
     }

    return (
        <>
        <section id="citySearchSection">
        <div id="citySearchMainDiv">
        {/* Logo Div */}
        <div id="logoDiv">
            <img id="logoImage" src="data:image/png;base64,"></img>
            <h3 id="bikeRentalsText">BikeRentals</h3>
        </div>

        {/* Search Bar */}
        <div id="searchDiv">
          <i id="searchIcon"><FaSearch/></i>
          <input id="seactInputBox" placeholder="Search or type city to Select"></input>
          <button id="searchClearBtn">Clear</button>
        </div>

        {/* City Names Div */}
        <div id="cityNameDiv">
      {cityData.map((city)=>{
        return (
            <>
            <div  id="cityDiv" onClick={()=>cityNameFun(city)}>
            <img  className="cityImage" src={city.imageUrl} ></img>
            <h3 className="cityName">{city.name}</h3>
            </div>
            </>
        )
      })}
        </div>

        <div id >
        <h3 id="selectTextHeading">SELECT CITY TO CONTINUE</h3>
        </div>
         
        </div>
        </section>
        
        </>
    )
}

export {CitySearch}